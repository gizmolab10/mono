...
# captured in memory:
#   1) yarn-not-npx — in this repo, run package binaries through yarn
...
if echo "$COMMAND" | grep -qE '(^|[[:space:]]|;|&|\||\()npx([[:space:]]|$)'; then
  REASON="BANNED COMMAND: the command uses npx. In this repo, run package binaries through yarn instead (e.g., \`yarn <bin>\` or \`yarn run <script>\`). Rewrite the command without npx."
  jq -n --arg reason "$REASON" '{hookSpecificOutput: {hookEventName: "PreToolUse", permissionDecision: "deny", permissionDecisionReason: $reason}}'
  exit 0
fi
