...
#   memory/di/truth/banned words.md
...
# Keep the log from growing forever: once it runs past the limit, drop the
# oldest lines. Writes to a temp file first so a reader never sees it half-cut.
trim_log() {
  local count
  count=$(wc -l < "$LOG_FILE" 2>/dev/null | tr -d ' ')
  case "$count" in *[!0-9]*|"") return;; esac
  [ "$count" -le "$LOG_KEEP" ] && return
  tail -n "$LOG_KEEP" "$LOG_FILE" > "$LOG_FILE.tmp" 2>/dev/null && mv "$LOG_FILE.tmp" "$LOG_FILE" 2>/dev/null
}
...
done < <(awk -F'|' '{h=$3;n=$5;m=$6;gsub(/^[ \t]+|[ \t]+$/,"",h);gsub(/^[ \t]+|[ \t]+$/,"",n);gsub(/^[ \t]+|[ \t]+$/,"",m);if(n==""||n=="Never")next;if(n ~ /^-+$/)next;print (((h=="y")&&(m==""))?"hard":"soft") "\t" n}' "${BANNED_FILES[@]}")
