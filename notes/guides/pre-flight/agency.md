...
12. **Test before done** — written is not working. Fix every error that surfaces, whoever caused it. Write the test for the thing being changed: a change in a server is tested by asking that server, never by asking whatever calls it. *Done means working.*
...
18. **Report, never remove** — a checker complaining about something you were not asked to change licenses a report, never a deletion. Fix the smallest thing actually wrong and leave every prop, field and export standing. *Unused today is somebody's next turn.*
