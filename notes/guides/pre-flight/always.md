...
27. **ALWAYS fix warnings** — editor/linter warnings count as errors to fix, not to excuse as "existing style". This includes markdown-lint (e.g. MD007 list-indentation), svelte-check warnings, and unused-symbol notices. When a warning surfaces on a file you touch, fix it in the same pass; never report a task done while leaving warnings behind.
