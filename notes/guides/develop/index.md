# Develop

Code style, patterns, and development practices.

## Contents

- [(newer than SDD) methodology](./(newer%20than%20SDD)%20methodology.md)
- [(newer than SDD) methodology](./(newer%20than%20SDD)%20methodology.md)
- [(newer than SDD) methodology](./(newer%20than%20SDD)%20methodology.md)
- [(newer than SDD) methodology](./(newer%20than%20SDD)%20methodology.md)
- [(newer than SDD) methodology](./(newer%20than%20SDD)%20methodology.md)
- [(newer than SDD) methodology](./(newer%20than%20SDD)%20methodology.md)
- [(newer than SDD) methodology](./(newer%20than%20SDD)%20methodology.md)
- [(newer than SDD) methodology](./(newer%20than%20SDD)%20methodology.md)
- [Adding a guide](add%20a%20file.md)
- [Aesthetics](./aesthetics.md)
- [Best.practices](../../../di/notes/guides/project/philosophy/best%20practices.md)
- [Best practices](./best%20practices.md)
- [Build Notes](./build notes.md)
- [Build](./build.md)
- [Conceptual composition](./conceptual%20composition.md)
- [Constants](./constants%20%26%20subtypes.md)
- [Creating A Design](./create a design.md)
- [Creating A Proposal](./create a proposal.md)
- [Css](./css.md)
- [Early di spec](../../../di/notes/guides/development/early%20di%20spec.md)
- [Hits System](./hits system.md)
- [Markdown](./markdown%20structure.md)
- [Migration](./migrate.md)
- [Port](./port.md)
- [Keep Shop](./keep shop.md)
- [Lessons](./lessons.md)
- [Refactoring](./refactor.md)
- [Running E2e Tests](../running%20e2e%20tests.md)
- [Running e2e tests](./running%20e2e%20tests.md)
- [Sections spec](./sections%20spec.md)
- [Sections](sections.md) — a stack owns the gaps between its sections, and a separator stands centred in each one.
- [Style](./style%20guide.md)
- [Tags hierarchy](./tags%20hierarchy.md)
- [Testing](../../../di/notes/guides/project/philosophy/unit%20testing.md)
- [Unit testing](./unit%20testing.md)

