...
- **Holds the lexicon.** When the project's vocabulary drifts, keep shop renames the file and the references across hooks, indexes, the map, and the memory files so the lexicon stays the source of truth.
