...
- ~~The rest of the spreadsheet endings.~~ **Done.** Eighteen now: the plain-text ones (csv, tsv also spelled tab, psv, dif, slk) stored as words and ready to read; the packed ones (seven Excel endings, ods, ots, gnumeric, numbers) as raw bytes, heavy, each needing its sheets read out; the accounting file (qb) the same; and the five old ones (Lotus wk1/wk3/wk4, Quattro Pro qpw/wb3) named on a list of their own so they can be dropped in one line if nobody has one.
