...
- [x] **Record shapes** — Document (id, blob ref, name, storage, kind, date), Tag, Tagging, Relationship (with sort position), Predicate, plus the storage / kind / record enumerations.
...
- [x] **Namespaced storage** — read/write one record kind's list, keyed by the active storage so they never collide; plus the active-storage setting.
...
- [x] **Registry** — one instance per storage, active-storage store, saved choice, the ring (local only for now).
