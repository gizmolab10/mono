...
Some things ji wants to remember are shared across every browser but aren't documents: a list of which chat exchanges are hidden, hand-set tags, a small settings blob, later maybe a little message channel. The browser's own storage is per-machine, so it can't carry these between computers. AnythingLLM already travels between machines (every browser reaches the same workspace), so it's the natural shared home — if it can hold plain data.
...
- **No general key-value store.** The only home for arbitrary text is a *document*.
- **Storing and embedding are separate steps.** You can save a document and simply not embed it — then it never enters search and can never be cited in an answer.
- **A document's body is unreadable through the API.** Reading a document back returns its *details only* (title, author, description, and so on) — never the stored text. So an un-embedded document's body can't be fetched back.
- **The details fields do round-trip.** A string tucked into the document's `description` when it's saved comes back byte-for-byte when read. This is the seam.
...
AnythingLLM keeps its own bookkeeping in a single SQLite database file on the machine (`server/storage/anythingllm.db`), managed through the Prisma toolkit. That database is the relational store — the lists and records the app runs on: the workspaces, one row per document (its name, its on-disk path, its details, whether it's pinned), the users, the API keys, the saved chat messages, and the system settings.

Two things live *outside* that database, and the split is exactly why the sideband trick behaves the way it does:

- **The embeddings** — the vectors a search matches against — go to a separate vector database (LanceDB by default), in its own folder on disk. Our note is never embedded, so it never touches this, and can never be a search hit or a citation.
- **Each document's actual text** is a parsed file on disk (a `documents` folder, with an embedded copy cached under `vector-cache`) — *not* a column in the SQLite database. The API's "read a document" reads only the SQLite row (the details), never the on-disk body. That's the whole reason an un-embedded note's `textContent` can't be read back, but its `description` can: the description *is* a text column on that SQLite row.

It also explains why the room is so generous — a note's payload sits in a plain text field of a SQLite table, and SQLite text has no small limit, which matches the 4 MB exact round-trip measured above.
...
**One small un-embedded document per kind of data**, named by a fixed marker, with the real payload (a JSON string) living in its `description`.

- **Write.** Save a raw-text document with the title set to a marker (e.g. `ji-state`) and the `description` set to the JSON payload. Do *not* embed it. It becomes `ji-state.txt` in AnythingLLM's own document folder.
- **Read.** Ask for the full document list, find the one whose title is the marker, take its stored name, read that document's details, and pull the payload out of `description`. (After a write, the returned location can be remembered so the next read skips the list step.)
- **Update.** A raw-text save always makes a *new* file, so replacing means: delete the old marker document, then save the fresh one. On read, if more than one marker document exists, keep the newest and clear the rest.

### Read-merge-write — the last writer folds in, never wipes

A plain "replace the note" is dangerous when two browsers change it. Say the note holds a hidden-list. Browser A reads it, hides exchange 5, and writes. Browser B — working from a copy it read a moment earlier, before A's change — hides exchange 9 and writes. B's write replaces the whole note with its own stale copy, and A's "hide 5" is gone. Last-to-write wiped information.
...
- **read(marker)** → the parsed payload, or nothing if the note isn't there.
- **write(marker, payload)** → replace the note with a fresh one.

Both log plainly: what marker, how many bytes, found or not, replaced or created.
...
- Replacing a note leaves exactly one marker document behind, not a growing pile.
