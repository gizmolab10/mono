...
- `operations/List_Documents.svelte` also hosts the LLM ask box (`support/Ask_LLM.svelte`) when the LLM store is active.
- `support/Ask_LLM.svelte` — the LLM store's ask box: a question field + ask button that sends the question to the local AnythingLLM and shows the answer plus the documents it drew from. Shown only when the LLM store is chosen.
...
- `managers/Filter_Documents.ts` — the shared filter: the picked-tags set, filter-text, match-mode, and picked-families stores, plus a `filter_rows` helper that keeps the content rows clearing every active filter (tags, text, family) and drops folders left with nothing under them.
