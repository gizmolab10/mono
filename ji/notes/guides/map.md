...
- `Persistence.ts` — the in-memory dirty-tracking manager (which record ids of each kind still need saving; the modify date lives on the Document itself). Named to free "Persistable" for the planned record base class — see [persistables plan](persistables.md).
