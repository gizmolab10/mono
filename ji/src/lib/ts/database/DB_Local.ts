...
import { T_Backend, T_Record } from './DB_Records';
import { preferences } from '../managers/Preferences';

// The local backend. Record lists live in browser storage, namespaced by this
// backend so they never collide with another's. The blob seam currently also
...
export class DB_Local extends DB_Common {
	readonly backend = T_Backend.local;

	protected load_list<T>(record: T_Record): T[] {
		return preferences.readDB<T>(this.backend, record);
	}

	protected save_list<T>(record: T_Record, list: T[]): void {
		preferences.writeDB<T>(this.backend, record, list);
	}

	write_blob(document_id: string, content: string): void {
		preferences.writeDB<string>(this.backend, `blob/${document_id}`, [content]);
	...
	read_blob(document_id: string): string | null {
		const held = preferences.readDB<string>(this.backend, `blob/${document_id}`);
		return held.length > 0 ? held[0] : null;
	}

	delete_blob(document_id: string): void {
		preferences.removeDB(this.backend, `blob/${document_id}`);
	}
}
