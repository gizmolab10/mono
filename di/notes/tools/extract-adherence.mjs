...
function format_dashboard(result, metrics, total_stipulations, total_tests, build_number, timestamp) {
	const lines = [];
	lines.push(`<!-- Generated by notes/tools/extract-adherence.mjs at ${timestamp}. Do not edit by hand. -->`);
	lines.push('');
	lines.push('# Adherence dashboard');
	lines.push('');
	lines.push(`As of build ${build_number ?? '(unknown)'}. The development process the project follows is at [logic driven design](./logic%20driven%20design.md).`);
	lines.push('');

	// thresholds:
	//   coverage     — every opted-in area at one stipulation per module or higher
	//   test binding — zero uncovered
	//   orphan tests — zero
	//   build-gate   — green
	const audited_areas = metrics.coverage_by_area.filter(r => r.modules > 0);
	const red_areas = audited_areas.filter(r => r.status === 'red').map(r => r.area);
	const coverage_green     = red_areas.length === 0;
	const test_binding_green = metrics.test_binding.uncovered === 0 && total_stipulations > 0;
	const orphan_green       = metrics.orphan_tests.count === 0;
	const build_gate_green   = metrics.build_gate.status === 'green';
	const overall_green = coverage_green && test_binding_green && orphan_green && build_gate_green;

	const failing = [];
	if (!build_gate_green)   failing.push(`build-gate (${metrics.build_gate.status})`);
	if (!test_binding_green) failing.push(`test binding (${metrics.test_binding.uncovered} uncovered)`);
	if (!orphan_green)       failing.push(`orphan tests (${metrics.orphan_tests.count})`);
	if (!coverage_green)     failing.push(`coverage (${red_areas.length} ${red_areas.length === 1 ? 'area' : 'areas'} below threshold: ${red_areas.join(', ')})`);

	if (overall_green) {
		lines.push(`**Overall:** 🟢 green`);
	} else {
		lines.push(`**Overall:** 🔴 red — failing: ${failing.join('; ')}`);
	}
	lines.push('');
	lines.push(`**Overall health:** ${result.legacy.length} ${result.legacy.length === 1 ? 'rule remains' : 'rules remain'} on the old shape.`);
	lines.push('');

	lines.push('## Migration progress');
	lines.push('');
	lines.push(`- Stipulations total: ${total_stipulations}.`);
	lines.push(`- Migrated to the new format: ${result.matched.length + result.uncovered.length}.`);
	lines.push(`- Still on the legacy "Covered:" format: ${result.legacy.length}.`);
	lines.push('');

	lines.push('## Test binding');
	lines.push('');
	lines.push(`- Current: ${metrics.test_binding.matched} matched, ${metrics.test_binding.uncovered} uncovered.`);
	lines.push(`- Target: zero uncovered.`);
	lines.push(`- Gap: ${metrics.test_binding.uncovered}.`);
	lines.push('');

	lines.push('## Orphan tests');
	lines.push('');
	lines.push(`- Current: ${metrics.orphan_tests.count}.`);
	lines.push(`- Target: zero.`);
	lines.push(`- Gap: ${metrics.orphan_tests.count}.`);
	lines.push('');

	lines.push('## Build-gate health');
	lines.push('');
	lines.push(`- Current: ${metrics.build_gate.status}.`);
	lines.push(`- Target: green.`);
	lines.push(`- Note: ${metrics.build_gate.note}.`);
	lines.push('');

	lines.push('## Coverage by area');
	lines.push('');
	lines.push('| Area | Stipulations | Modules | Coverage | Status |');
	lines.push('| --- | ---: | ---: | ---: | --- |');
	for (const row of metrics.coverage_by_area) {
		const cov = row.coverage === null ? '—' : (row.coverage * 100).toFixed(0) + '%';
		const status_label = row.status === 'green' ? '🟢' : row.status === 'red' ? '🔴' : 'not yet audited';
		lines.push(`| ${row.area} | ${row.stipulations} | ${row.modules} | ${cov} | ${status_label} |`);
	}
	lines.push('');

	if (result.malformed.length > 0) {
		lines.push('## Malformed entries');
		lines.push('');
		for (const m of result.malformed) {
			lines.push(`- ${m.file}:${m.line} — ${m.reason}`);
		}
	...
	lines.push('---');
	lines.push('');
	lines.push(`*As of build ${build_number ?? '(unknown)'}, generated by the adherence script.*`);
...
function main() {
	...
	const dashboard = format_dashboard(result, metrics, stipulations.length, tests.length, build_number, timestamp);
