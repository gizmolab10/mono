...
- [ ] keep the granularity steady
    - [ ] one stipulation per axis of behavior, not per line
...
- [ ] codify the workflow for new work
    - [ ] no new feature is added without a stipulation in its area
    - [ ] no stipulation is added without a test
    - [ ] no test is added without a passing run
- [ ] carve out a fixes exemption
    - [ ] a bug fix needs a test that demonstrates the fix
    - [ ] a new stipulation is added only when the bug reveals an invariant nobody had written down
...
Three hand-recorded metrics, captured in [adherence log.md](../../development/adhere/adherence%20log.md), one append-only line each:

- **Re-read recency** — date of last sweep, drifters surfaced, drifters resolved.
- **New-work compliance** — per merged change: was a rule added first, was a test added alongside, did a passing run precede merge.
- **Failure triage** — per failure: outcome (code wrong, rule wrong, both wrong, unclear-pause), ticket reference.

The auto dashboard lives at [adherence dashboard.md](../../development/adhere/adherence%20dashboard.md), regenerated by `yarn adherence`. The reading guide is at [dashboard guide.md](../../development/adhere/dashboard%20guide.md).

We deliberately do not measure the visual or emotional quality of any feature (those live in the design-notes register, never gated by the build), or the collaborator's per-turn behavior (cadence and propose-first compliance is a human-side observation; logged only when a violation surfaces).

## adding a new rule

When a new rule is added — from feature work or from an audit sprint that exposes a missing invariant — eight steps:
