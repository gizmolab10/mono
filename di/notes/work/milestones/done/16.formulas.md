...
The complete test plan lives in each phase's "What gets tested" subsection further down — every test is attached to the phase that puts it in place. The whole-feature acceptance set passes when phase two ships, which is the moment the feature reaches end users.
...
Four phases. Phase zero, phase one, and phase two are all required for the feature to ship to users. Phase three is observability polish and can be skipped or deferred. Phases one and two split the work for clean code review and clean revert paths, but they are not separately shippable to users — phase one without phase two leaves drags failing silently, which is a usability bug. Phase one can sit on a development branch on its own; it cannot reach end users without phase two also landing.

#### Phase 0 — the new status strip (shipped 2026-04-28)

A short on-screen surface that displays brief transient messages. Shipped on 2026-04-28 as its own piece of work. The first iteration carries a placeholder caller hooked under the name `di_status` so a developer can fire messages from the browser console; the center-letter work in phase two will be the first real caller and the placeholder comes out then.
...
This phase is shipped. The center-letter work can now proceed to phase one whenever a developer picks it up; phase two will wire the silent refusal in phase one to the strip that already exists.

##### Risks specific to phase zero (retrospective)

The five risks below were the picture before the work shipped. After the work landed: none of them surfaced as real bugs. The notes are kept here as a reference for the reader who wants to understand the design choices.
...
This phase is reviewable and revertable on its own as a code-change unit. It is **not** shippable on its own to end users: the silent refusal of a drag is a usability bug. Phase two must land before this work reaches users.
...
- **[low] Phase two without phase zero.** The implementation steps in phase two assume the strip exists. If a developer skips phase zero and tries to ship phase two anyway, the publish call lands on nothing visible. Mitigation: a build-time check or a console warning if the strip is missing when a publish call fires.
...
Phase zero is shipped. Phase one and phase two are still required for the feature to reach end users. Phase three (debug logs and optional tooltips) is observability polish and can be skipped or deferred without affecting the user-visible feature.
