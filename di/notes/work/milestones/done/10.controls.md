...
- [x] add a sep between hamburger and other_buttons
- [x] move edit into other_buttons
- [x] three control rows for smart phones (narrower than 420)
	- [x] first ham, other
	- [x] second face_buttons
	- [x] third mode_buttons
...
- [x] add a button that toggles between 2D and 3D, associated with a store
- [x] add a button called "straighten" that sets the root SO's orientation to identity
- [x] button for precision during drag (avoid 3' 8 47/64")
- [x] straighten -> apply to forward face of root
- [x] switch 3D -> 2D
  - [x] wrong face
  - [x] wrong twist
  - [x] child projections wrong
  - [x] missing some dimensionals
- [x] change see through -> x-ray
- [x] add vertical separator
...
| `src/lib/ts/render/Setup.ts` | restore scale on load, wire wheel, export `scale_up`/`scale_down` |
| `src/lib/ts/runtime/Smart_Object.ts` | serialize/deserialize now include scale |
| `src/lib/ts/managers/Scenes.ts` | `Saved_Scene` interface updated |
...
| `src/lib/ts/render/Setup.ts` | `set_scale`, `get_scale` exports |
...
| File | What |
| ---- | ---- |
| `src/lib/ts/render/Setup.ts` | `w_scale` store, subscriber syncs O_Scene + persists, removed `set_scale`/`get_scale` |
...
- [x] add a six segment control to controls, one for each face
  - [x] darken segment corresponding to forward face of root SO
...
- [x] make controls double height for narrow screen (defined as "they don't fit")
- [x] rearrange so they fit narrow screen
