...
| `3px` border-radius | input-wrap, between `--r-input` and `--r-box` | P_Selected |
...
| `--c-default`         | `colors.default`    | `'black'`                      |
...
- `white` → `var(--white)` (~50 instances)
- `black` → `var(--c-default)` (~25 instances)
...
- font sizes — converted to `--font-small` / `--font-common` CSS vars
- border thickness — `--th-border` (0.5px)
- input corner radius — `--r-input` (2px)
- small gap / small padding / letter spacing — `--l-gap-small`, `--l-padding-small`, `--l-letter-spacing`
- static colors — `--white`, `--c-default`, `--c-focus`, `--c-thumb`, `--c-track`, `--c-slider-border` (via Colors.ts)
...
Constants.ts (design tokens: all measurements derived from common_size)
    ↓
App.svelte onMount (injects --l-gap, --h-controls, --r-common, --text, --accent, --bg onto :root)
    ↓
