...
class Evaluator {
	...
	propagate(node: Node, target: number, resolve: Resolver, write: Writer): void {
		const refs = this.collect_references(node);
	...
	detect_cycle(formulas: FormulaMap): string[] | null {
		...
		for (const key of formulas.keys()) {
			const cycle = this.dfs(key, formulas, visited, in_stack, []);
	...
	private solve_for_reference(node: Node, ref: Ref, target: number, resolve: Resolver): number {
		switch (node.type) {
			...
			case 'reference':
				if (this.refs_match(node, ref)) return target;
			...
			case 'binary': {
				const ref_in_left = this.contains_reference(node.left, ref);
				const ref_in_right = this.contains_reference(node.right, ref);
				...
				if (ref_in_left) {
					...
					const left_target = this.invert_left(node.operator, target, right_val);
					...
					const right_target = this.invert_right(node.operator, target, left_val);
	...
	private collect_references(node: Node): Ref[] {
		switch (node.type) {
			case 'literal': return [];
			case 'reference': return [{ object: node.object, attribute: node.attribute }];
			case 'unary': return this.collect_references(node.operand);
			case 'binary': return [...this.collect_references(node.left), ...this.collect_references(node.right)];
		}
	}

	private refs_match(a: Ref, b: Ref): boolean {
		return a.object === b.object && a.attribute === b.attribute;
	}

	private contains_reference(node: Node, ref: Ref): boolean {
		switch (node.type) {
			case 'literal': return false;
			case 'reference': return this.refs_match(node, ref);
			case 'unary': return this.contains_reference(node.operand, ref);
			case 'binary': return this.contains_reference(node.left, ref) || this.contains_reference(node.right, ref);
		}
	}
	...
	private dfs(
		...
		if (node) {
			const deps = this.collect_references(node);
			for (const dep of deps) {
				const dep_key = nodes.ref_key(dep.object, dep.attribute);
				const cycle = this.dfs(dep_key, formulas, visited, in_stack, path);
				if (cycle) return cycle;
			}
		}
	...
	private invert_left(op: string, target: number, right_val: number): number {
		switch (op) {
	...
	private invert_right(op: string, target: number, left_val: number): number {
		switch (op) {
