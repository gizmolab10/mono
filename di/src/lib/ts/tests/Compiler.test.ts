...
import { tokenize } from '../algebra/Tokenizer';
...
describe('Tokenizer', () => {
	...
	it('tokenizes references', () => {
		const tokens = tokenize('wall.height');
	...
	it('throws on bare identifier without dot', () => {
		expect(() => tokenize('wall')).toThrow(/Expected '.' after/);
	});

	it('throws on reference with missing attribute', () => {
		expect(() => tokenize('wall.')).toThrow(/Expected attribute name/);
	});
});
...
describe('Compiler', () => {
	...
	it('throws on unclosed paren', () => {
		expect(() => compile('(1 + 2')).toThrow(/Expected '\)'/);
	});
	...
	it('throws on unexpected token after expression', () => {
		expect(() => compile('1 2')).toThrow(/Unexpected token/);
	});
});
