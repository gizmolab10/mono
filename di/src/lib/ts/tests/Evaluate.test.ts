...
import { evaluate, propagate, detect_cycle } from '../algebra/Evaluate';
import type { Resolver, Writer, FormulaMap } from '../algebra/Evaluate';
...
describe('evaluate', () => {
	...
	it('evaluates a literal', () => {
		const node = compile('42');
		expect(evaluate(node, mock_resolver({}))).toBe(42);
	});

	it('evaluates a reference', () => {
		const node = compile('wall.height');
		const resolve = mock_resolver({ 'wall.height': 2438.4 });
		expect(evaluate(node, resolve)).toBeCloseTo(2438.4);
	});

	it('evaluates addition', () => {
		const node = compile('wall.height + 100');
		const resolve = mock_resolver({ 'wall.height': 2438.4 });
		expect(evaluate(node, resolve)).toBeCloseTo(2538.4);
	});

	it('evaluates subtraction with unit literal', () => {
		// wall.height - 6" (6" = 152.4mm)
		const node = compile('wall.height - 6"');
		const resolve = mock_resolver({ 'wall.height': 2438.4 });
		expect(evaluate(node, resolve)).toBeCloseTo(2286);
	});

	it('evaluates multiplication', () => {
		const node = compile('2 * door.width');
		const resolve = mock_resolver({ 'door.width': 914.4 });
		expect(evaluate(node, resolve)).toBeCloseTo(1828.8);
	});

	it('evaluates division', () => {
		const node = compile('wall.width / 2');
		const resolve = mock_resolver({ 'wall.width': 3048 });
		expect(evaluate(node, resolve)).toBeCloseTo(1524);
	});

	it('evaluates division by zero as 0', () => {
		const node = compile('wall.width / 0');
		const resolve = mock_resolver({ 'wall.width': 3048 });
		expect(evaluate(node, resolve)).toBe(0);
	});
	...
	it('evaluates complex expression with precedence', () => {
		...
		const node = compile('2 * door.width + 10 mm');
		const resolve = mock_resolver({ 'door.width': 914.4 });
		expect(evaluate(node, resolve)).toBeCloseTo(1838.8);
	});

	it('evaluates parenthesized expression', () => {
		// (wall.height + 100) * 2 = (2438.4 + 100) * 2 = 5076.8
		const node = compile('(wall.height + 100) * 2');
		const resolve = mock_resolver({ 'wall.height': 2438.4 });
		expect(evaluate(node, resolve)).toBeCloseTo(5076.8);
	});
});
...
describe('propagate', () => {
	...
	it('propagates through subtraction: a = b - 6" → change a, update b', () => {
		...
		propagate(node, 2286, resolve, writer);
	...
	it('propagates through addition: a = b + 100', () => {
		...
		const { writer, writes } = mock_writer();
		propagate(node, 2538.4, resolve, writer);
		expect(writes['wall.height']).toBeCloseTo(2438.4);
	});
	...
	it('propagates through multiplication: a = 2 * b', () => {
		...
		const { writer, writes } = mock_writer();
		propagate(node, 1828.8, resolve, writer);
	...
	it('propagates through division: a = b / 2', () => {
		...
		const { writer, writes } = mock_writer();
		propagate(node, 1524, resolve, writer);
	...
	it('propagates through nested ops: (b + 10) * 2 == target', () => {
		...
		const node = compile('(wall.height + 10) * 2');
		const resolve = mock_resolver({ 'wall.height': 0 });
		const { writer, writes } = mock_writer();
		propagate(node, 200, resolve, writer);
		expect(writes['wall.height']).toBeCloseTo(90);
	});

	it('throws on no references', () => {
		const node = compile('42 + 10');
		const resolve = mock_resolver({});
		const { writer } = mock_writer();
		expect(() => propagate(node, 100, resolve, writer)).toThrow(/No references/);
	});

	it('throws on multiple references', () => {
		const node = compile('wall.height + door.width');
		const resolve = mock_resolver({ 'wall.height': 100, 'door.width': 50 });
		const { writer } = mock_writer();
		expect(() => propagate(node, 200, resolve, writer)).toThrow(/Multiple references/);
	});
});
...
describe('detect_cycle', () => {
	...
	it('returns null for no formulas', () => {
		...
		expect(detect_cycle(formulas)).toBeNull();
	});

	it('returns null for acyclic formulas', () => {
		const formulas: FormulaMap = new Map();
		// door.height = wall.height - 6"
		formulas.set('door.height', compile('wall.height - 6"'));
		expect(detect_cycle(formulas)).toBeNull();
	});
	...
	it('returns null for chain without cycle', () => {
		...
		formulas.set('a.x', compile('b.x + 10'));
		formulas.set('b.x', compile('c.x * 2'));
		expect(detect_cycle(formulas)).toBeNull();
	});
	...
	it('detects direct cycle: A → B → A', () => {
		const formulas: FormulaMap = new Map();
		formulas.set('a.x', compile('b.x + 10'));
		formulas.set('b.x', compile('a.x - 10'));
	...
	it('detects indirect cycle: A → B → C → A', () => {
		const formulas: FormulaMap = new Map();
		formulas.set('a.x', compile('b.x + 1'));
		formulas.set('b.x', compile('c.x + 1'));
		formulas.set('c.x', compile('a.x + 1'));
		const cycle = detect_cycle(formulas);
		expect(cycle).not.toBeNull();
	...
	it('detects self-referencing formula', () => {
		const formulas: FormulaMap = new Map();
		formulas.set('a.x', compile('a.x + 1'));
		const cycle = detect_cycle(formulas);
		expect(cycle).not.toBeNull();
	...
	it('only flags the cycle, not unrelated formulas', () => {
		const formulas: FormulaMap = new Map();
		formulas.set('a.x', compile('b.x + 1'));
		formulas.set('b.x', compile('a.x + 1'));
		formulas.set('c.x', compile('d.x + 1')); // unrelated, acyclic
		const cycle = detect_cycle(formulas);
		expect(cycle).not.toBeNull();
