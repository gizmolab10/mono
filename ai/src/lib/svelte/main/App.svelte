<script lang='ts'>
	...
	// The host of kb's page: draws it, and hands it three snippets. The edit filter section since
	...
	// other one comes with its step.
	//
	// The drawer and the search are wired here, both being ai's: the drawer hands its html up, the
	// search looks through it, and a file drawn or drawn again tells the search so.

	// The drawn words, handed up by the drawer, so the search can look inside them.
...
<Main {edit_filter} {search_row} {operation_view} />
