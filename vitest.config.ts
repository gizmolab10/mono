...
// names the real suites instead: the two with configs of their own run under them,
// and the three plain-TypeScript suites get a project each. Nothing else is collected.
export default defineConfig({
	test: {
		projects: [
			'projects/di/vitest.config.ts',
			'projects/ws/vitest.config.ts',
			{ test: { name: 'ji', include: ['projects/ji/src/**/*.test.ts'] } },
			{ test: { name: 'lv', include: ['projects/lv/src/**/*.test.ts'] } },
