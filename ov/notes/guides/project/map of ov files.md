...
- `Hideable.svelte` — one thing in the details column that folds away behind a word on a line, the same shape every other folding thing wears: it builds its own word, styles it, and hands the made element to the line, which only finds it a place to stand. It asks its section for half the heavy line on top of the gap above, so both sides draw as one gap and what it shows sits centred.
...
- `utilities/Saving.ts` — writing a changed file back to its own place. Works out where a guide, a design or a work note sits counting from the top of the repo, and the other way round; where a file lands when it is dropped into a folder or given a different name; and hands the dispatcher the whole new text along with the text as it was when the file was opened — which that server checks against the file before writing anything. Also asks the dispatcher to start itself over, and waits until it answers again.
...
- `utilities/Tooltip.ts` — the plumbing behind the hover hint: a store holding what's pointed at, and one watcher that follows the cursor. It asks the hits manager first, since that names the one thing under the cursor; only where nothing there carries words does it walk up the page for the nearest that does. **⟵ji**
- `events/Hits.ts` — who is under the cursor, and what a press on them means. Every part of the app the mouse can reach hands over one target holding the rectangle it stands in; they are kept in a structure that answers "what is at this point" without asking each in turn. A control beats a section, which beats the file's own words, and within one kind the one standing in the smaller area wins. Also the beat behind a mark held down, and the log line when two things claim one name. **⟵di**
- `events/Hit_Target.ts` — one element told to answer the cursor, said in one place. It makes the target, hands over the rectangle, stamps the element while the manager names it, and takes the target away when the element goes. A thing drawn out of sight says so and holds no place at all. **⟵di**
...
- `sectioning.test.ts` — how thick a boundary's line is drawn, the gap a section holds above and below what it shows, that gap measured from the line's middle, and how tall a folded section stands.
