...
// Two bridges reach through the "core" alias, and only two: common/Core.ts for everything
...
// here instead, where a third bridge fails it.
...
describe('the bridges through the core alias', () => {
	...
	it('are exactly two files', () => {
		expect([...bridges.keys()].sort()).toEqual(['src/lib/ts/common/Core.ts', 'src/lib/ts/main.ts']);
	});

	it('let main.ts reach for the stylesheet and nothing else', () => {
		expect(bridges.get('src/lib/ts/main.ts')).toEqual(['core/main.css']);
	});
