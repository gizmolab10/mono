...
## Completed — additional changes during implementation

**Circular dependency fix:** `Ancestry.ts` now imports `databases` from `Databases.svelte.ts`, which imports `DB_Firebase` extending `DB_Common`. `DB_Common` and `Databases.svelte.ts` previously imported `rootAncestry` from `Ancestry.ts` — creating a cycle. Fix: both use `await import('../nav/Ancestry')` in their async methods instead of top-level imports.

**Ancestry statics:** Module-level cache functions (`ancestry_remember_createUnique`, `ancestry_forget`, `ancestry_forget_all`) and `rootAncestry` const moved to static members of the `Ancestry` class: `Ancestry.remember_createUnique()`, `Ancestry.forget()`, `Ancestry.forget_all()`, `Ancestry.root`.

**Persistent statics:** `data_isValidOfKind()` moved to `Persistent.isValidOfKind()`. Redundant static alias on `DB_Firebase` removed.

**G_Widget statics:** `_ctx`, `getContext()`, `dot_size`, `width_ofDrag` moved from module level to private static members.

**Actual consumers updated:** `D_Selection.svelte` (not `Events.svelte.ts` as originally listed).
