...
i wanted one place that says where unfinished work sits, across every project, without reading every file. `tools/unfinished.py` writes it: [unfinished.md](../zone/unfinished.md), one line per memory file that holds unfinished work. The shorthand `unfinished` runs the script and replies with the count of lines and the file's path.
...
Every project folder under memory, every markdown file however deep, unfinished.md itself left out. The folders archive, done and node_modules are skipped. A logs folder is walked, since the project's log has sat there from 13 September 2026.
...
`tools/test_unfinished.py` feeds the script a made-up project folder and checks every pattern, 15 checks. The proposal that made it is the section big picture, 9 September 2026, of [proposals.md](../zone/proposals.md), its name until 13 September 2026.
