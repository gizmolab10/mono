...
Copy overview's [Stack.svelte](../../../../../notes/src/lib/svelte/support/Stack.svelte) whole, then mend the five imports at its top. Change nothing else. Every number in it is worked out from the props and the four ladder rungs.
...
Four rules decide a fold, and nothing else:
...
7. **A clickable at a line's middle needs a background of its own.** It hangs down past the line, and where the section below is folded that is a run of accent — so without a page-colored pill behind it the accent shows through and it stops reading as standing on the line.
...
Ask the app to report what it drew, then read the log. Overview's own report is [Separator_Spacing.ts](../../../../../notes/src/lib/ts/utilities/Separator_Spacing.ts):
