...
Meta-lessons distilled from the two mothballed milestones (the facet tracer and the drag rewrite). Each is a pattern that would mislead a successor on any feature, not just the one that surfaced it. The point is to spot the same pattern early when it shows up somewhere new.

For the case-level writing — exactly what was tried, what failed, and why — see [the facet-tracer lessons file](../../../di/zone/work/milestones/done/32.facets/lessons.md) and [the drag lessons file](../../../di/zone/work/milestones/33.drag/lessons.md).
...
The pattern: when two desirable behaviors are written down in user-facing terms and both feel intuitive, check whether they are mathematically compatible before promising both. Often the only way out is to give up one or pick a different pivot.
