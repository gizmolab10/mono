...
- 31 August 2026; **move `questions.md` into `zone/`.**
  Weighed once already: the 29 August pac asked this bundled with `log.md` and was never decided, its deciding question left open — is zone "ideas before belief", as the lexicon then said, or "everything not yet truth"? Two things have changed since. Zone was redefined without a formal sweep — `shared/index.md` now reads "zone means a project's live thinking — ideas.md for ideas, proposals.md for proposals, drive.md for the one being driven" — and a proposal is not an idea before belief, so the wider reading already won in practice. And this ask is the narrower half: the log stays at the root, which drops the old pac's strongest objection, that a log is a time layer and a peer of truth/, not a maybe.
  For: a question is unsettled by definition, and zone is the one door for the unsettled — the root then reads as three plain layers, what is believed (truth/), what happened (log.md), what is not settled (zone/). A question usually feeds the drive it sits beside, and today proved it: every question in shared moved by hand into `zone/drive.md`, so the two were already one thing. The handbook's "the file exists only while questions do" also reads better inside zone, where files come and go by design, than at a root where a vanishing file looks like breakage.
  Against: the file was put at the root to be seen — "questions are not history, and a log buries them" (handbook line 18) — and start reads the index, the lexicon and the matched truths, never the whole zone folder, so a question in zone is a question nobody opens. One name, one concept cuts the same way: live thinking is thinking being done, and a question is thinking stopped, waiting on Jonathan; admitting it needs zone formally redefined through `define` and swept, which the 29 August pac already named as the price. The root shrink is small — four doors become three, not one.
  The cost, counted: four files move (shared, mu, me, lv), and five pointers re-point — handbook lines 18, 20, 27 and 106, and `shared/index.md` line 8. Shorthand names no path, so it needs nothing.
  Deciding question: today every question in shared was moved by hand into `drive.md`. If that is the real answer, the question is not where `questions.md` sits but whether it exists at all — does a project keep a separate waiting list, or does each question ride the drive it belongs to?
  Decided 31 August 2026: the separate waiting list keeps its life and moves — all four questions files are now `zone/questions.md`, and zone holds four named files: ideas, proposals, drive, questions.

- 31 August 2026; **merge memory/shared into memory/core, or keep them separate.**
...
- 29 August 2026; **subfolders under truth/ and zone/.**
  ...
  Against: findability is the design's second goal — one glance at a flat truth/ is the whole catalog, and index.md already does the grouping a subfolder would do, so depth duplicates the index's job while adding a hunt step. The sizing caps exist to force merges and deletes; subfolders relieve exactly the pressure that keeps the corpus small. And two plausible homes (voice.md vs prose/voice.md) is the seed of the duplication bug.
  The numbers today: shared/truth holds 7 files, ov/truth 5 — half the cap. Structure ahead of need.
...
- 29 August 2026; **move open questions.md and log.md into zone/.**
...
- 27 August 2026; **ops** as the name for the control surface.
