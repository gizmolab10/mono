...
- **SOT** — source of truth: the one file that holds a fact, which every other mention points at. conventions.md is the SOT for the Response rules. Not: a copy, a summary, or what co remembers of the file.
...
- **correction** — a lesson Jonathan gave co: one checkbox line in `zone/learn.md`, captured in co's words, rewritten when Jonathan puts it in his and ticks it, placed when record moves it into its truth and writes a row in `logs/distilled.md`. Decided 17 September 2026, the row 18 September.
...
- **learn** — the task, and the shorthand of that name, that captures a lesson Jonathan gave co as one checkbox in `zone/learn.md`, the file of corrections: shared's for lessons that apply everywhere, a project's for its own. Held corrections.md's checkboxes since 18 September 2026; nothing raw, nothing to distill.
- **lexicon** — this file.
...
- **evidence** — a line number and a file name. Do not reveal until Jonathan asks you to describe the evidence. See always rule 3 in [conventions](conventions.md).
