...
A draft guide rule is born from a correction Jonathan made —> a checkbox item describing what co does or should do goes into a new file [[corrections]] shared-z.
...
1. **The names.** The shared lexicon gets **guide rule** and **draft guide rule**, as the section says them, with one D: line. Not: a rule of the app, the rules section of the details column. Not done: correction took their place, and guide debt is in no lexicon either.
2. **The learn row** says the rule is written as a checkbox, replacing a close line. Done, writing corrections.md instead of the guide.
3. **The 23 drafts** become checkbox lines where they sit: 13 in conventions.md Response 10 and 11, 7 in write a plan.md, 3 in pitfalls.md 22 to 24, each entry's body as one box under its heading. Not done.
4. **Success:** after `debt`, code debt.md lists conventions.md with 13 open truths, write a plan.md with 7 and pitfalls.md with 3, 23 in all. Code debt names files and counts, not the rules' words. Never ran, because 3 never ran.
