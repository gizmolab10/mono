...
- [ ] turn-taking
    ...
    - [ ] co researches, traces, proposes, and on a green light builds
...
We deliberately do not measure the visual or emotional quality of any feature (those live in the design-notes register, never gated by the build), or co's per-turn behavior (cadence and propose-first compliance is a human-side observation; logged only when a violation surfaces).
