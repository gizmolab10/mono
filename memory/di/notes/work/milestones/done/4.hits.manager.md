...
- [x] thumb buttons on sliders
	- [x] all sliders should use the hit target system
		- [ ] same color scheme
		- [ ] same thumb size
	- [x] P_Repeat gap_min / gap_max sliders — `pointer-events: none` on input blocks `:hover` state; thumb hover CSS won't fire; needs different approach
	- [x] thumb on scaling corrupts guides slider — new, undiagnosed
- [x] and segmented controls

## Goal

Adopt and deploy the hits manager from ws (webseriously) to di. Use it to create a uniform UX
