...
**Started:** 2026-02-07 **Status:** phases 1–6 done — 631 unit tests passing across 29 files; rules catalog covers every load-bearing algebra behavior with at least one test
...
- [x] add a center letter `c` — read-only midpoint between the start and the end of a direction
	- [x] resolver returns start-plus-end-over-two on every read; nothing stored
	- [x] both write paths refuse a write through a center reference
	- [x] a drag whose formula reads a center is refused with the message "cannot drag a center" on the on-screen status strip
	- [x] cycle detection rejects a formula on a start, end, or length cell that references the same-direction same-SO center
	- [x] center letter survives the round-trip between the explicit form and the axis-agnostic form
	- [x] save format unchanged — formulas store the bare letter, not its expansion
- [x] named values (givens) that any formula can reference
	- [x] a global table of named numbers; formulas resolve a bare name to that table when no part has the name
	- [x] a locked named value refuses reverse propagation, just as a locked cell does
- [x] error classification with span and message
	- [x] every parse failure carries a message and a character span into the source
	- [x] the failing cell shows a red underline under the bad span and the message on hover
	- [x] errors persist on the cell until the cell is edited or cleared
...
- [x] **5. names of attributes** in formulas
	- [x] x, y, z, w, h, d — nine customer aliases (see [[algebra]])
	- [x] X, Y, Z for far edges — no alternatives needed, uppercase is intuitive
	- [x] compound imperial: `1 1/2"`, `5' 3"`, `5' 3 1/2"`
	- [x] offset model: empty formula = `parent.attr + offset`
	- [x] invariant clearing: set_invariant and rebind_formulas both clear stale formulas