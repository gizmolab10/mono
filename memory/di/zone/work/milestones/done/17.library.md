...
- [x] 2 new buttons below list (adding to import, reinstall)
	- [x] replace -- loads selected library item as new scene
	- [x] insert -- adds selected item as child of selected SO (or root)
		- [ ] writes .s and .l formulas on the two non-forward axes
	- [x] click selects row, buttons act on selection
...
- [x] decide where the files are stored/uploadable from
	1. bundled defaults are packaged at `src/assets/*.di` via glob import
