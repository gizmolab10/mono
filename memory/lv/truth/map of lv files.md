...
- [Main.ts](../../src/lib/ts/Main.ts) — starts the app: pushes core's sizes, layers and inks onto the page, then mounts App
- [Vite_Env.d.ts](../../src/Vite_Env.d.ts) — the one line that tells the checker what vite's own import suffixes yield
...
- [Core.ts](../../src/lib/ts/common/Core.ts) — everything lv adopts from core, one line each, through the "core" alias. Only this file reaches through it; every other lv file imports here. The hamburger and the section come this way too
- [Customizations.ts](../../src/lib/ts/common/Customizations.ts) — lv's own switches, gathered into one value so a caller names the file rather than every switch. `enable_sidebar` is the first
...
- [Sidebar_Content.ts](../../src/lib/ts/utilities/Sidebar_Content.ts) — the page list, built from every page's own top settings
- [S_Sidebar.svelte.ts](../../src/lib/ts/utilities/S_Sidebar.svelte.ts) — whether the sidebar is showing
...
- [movie title.test.ts](../../src/lib/ts/test/Movie_Title.test.ts) — 9. A movie's blocks walked, which movies take a caption, the write, and the picture data left where it was
- [loader.test.ts](../../src/lib/ts/test/Loader.test.ts) — 9. Every page and every picture gathered, keyed by name
- [sidebar-content.test.ts](../../src/lib/ts/test/Sidebar_Content.test.ts) — 9. The page list built from every page's top settings
