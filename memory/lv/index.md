...
**Current state:** lexicon folded in from CLAUDE.md — the migration's first move (changed to inception, on 28 August 2026); the gallery truth stands current; the old-notes code debt now leads `zone/ideas.md`, its done section left to git; the map lives in `truth/`. Open: which remote hosted storage, if the pictures outgrow the repo.
