...
`gallery/netlify/functions/add-photo.mts`. The file arrives in the request itself, the caption is written inside it, and it is committed. A photo from a phone fits; anything larger is refused with a line saying so, by the page before it sends and by the function after it arrives.
...
4. **The function fetches the file from the remote hosted storage**, writes the caption inside it, and commits it to `gallery/src/assets/<folder>/<name>` on `main`.
