...
- **Small files.** Truths stay under ~100 lines. When a file outgrows its concept, split it and update the index.
...
└── <project>/               # one folder per project (di/, ws/, musicology/ …)
    ...
    ├── truth/               # one concept per file; the ONLY place a fact lives; may hold folders
    │   ├── architecture.md
    │   ├── data-model.md
...
Two levels deep, everywhere, though truth may hold folders of its own. `shared/` is just a project whose subject is you. The three content folders complete a set: truth is believed, zone is not yet believed, archive is no longer believed.
...
2. Frontmatter grep — `tags` and `description` across `truth/`, folders included.
