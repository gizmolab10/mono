...
Projects allow more than one pick. Empty means all. The project column shows unless exactly one is picked. The table ends with a right-justified size column at `--font-tiny`: a file's character count read at launch, a folder summing its matching files, written through `in_thousands` (38392 → 38.4k), sortable with numbers padded so 9 sorts before 80. The kinds row offers only kinds with matches; a picked kind that loses its last match clears itself.
