...
It reaches outside its own folder on purpose: the guides are the thing it shows. What travels with it is only their addresses. At launch it hangs every file from the dispatcher's listing and draws the list at once, then reads each file once — the file being edited first, then the rows in view, then the rest, a few at a time — keeps the five labels off the top, and lets the rest go, the list narrowing as each batch arrives; so the picture cannot drift from the files, because it *is* the files, read fresh every time.
