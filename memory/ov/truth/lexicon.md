...
The shared [banned words](../../shared/notes/guides/pre-flight/banned%20words.md) list holds the words that mean nothing to Jonathan, each with the word to use instead; ov's own [banned words](../notes/guides/pre-flight/banned%20words.md) does the same for this project. The words every project says — guide, work note, collection, labels, kind, tag, brief — are not repeated here.
