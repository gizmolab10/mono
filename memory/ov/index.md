...
- [adopt kb.md](zone/adopt%20kb.md) — what a host hands kb, proposed: the switches, one field per fact with ai's and mu's values, the four snippets and where kb renders each, what is not handed, the type, its proof and what is open. Moves to memory/kb at strip step 1.
- [music and ai.md](zone/music%20and%20ai.md) — kb, the library extracted from ov, which mu and ai import, each bringing its specialty: what kb holds, what a specialty is, the ai and music specialties each defined with a key values table, music with its open questions, the db tables, and the plan: the order, music first on the dispatcher side, then ov strip down, then the music phases, and a proposal to thread the dispatcher without a mutex.
