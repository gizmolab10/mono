...
- [knowledge bases.md](zone/knowledge%20bases.md) — how companies organize and browse their knowledge, and which of those approaches fit ov, mu and ji. Its roadmap's phases 1 to 4 are built: the db beside the dispatcher holds all five labels of every file, the files list and the editor read it, no memory file carries a block, and the dispatcher watches the disk so a file moved in the Finder keeps its labels.
