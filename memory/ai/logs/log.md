...
- D: how ai hosts kb.md and ai's index name hosting kb.md in mu's zone, the file having moved. Jonathan's word
- D: ai's Customizations.ts comment names memory/kb/truth/adopt kb.md, the file having moved. Jonathan's move
- D: truth/design/how ai hosts kb.md, moved from ai's truth through the dispatcher; the design index, the truth index and ai's index say so; the links at it re-pointed. Jonathan's word
- D: truth/how ai hosts kb.md moved here from kb's truth through the dispatcher; the truth index and ai's index list it; its hosting kb link re-based; working features row 104 and the journal's Current line point here. Jonathan's word
- D: how ai hosts kb.md and ov as knowledge bases.md moved to kb's truth through the dispatcher; the work index and ai's index no longer hold them. Jonathan's word
- D: adopt kb.md moved to kb's zone and kb hosted by music.md to mu's, through the dispatcher; the work index and ai's index no longer list them, kb hosted by ai.md names music's file in mu's zone. Jonathan's word
- D: kb hosted by ai.md is ai's part of the plan alone, 51 lines, and kb hosted by music.md music's, 84, the kb material gone to kb's zone/hosting kb.md; each names the others; the work index and ai's index list both. Jonathan's go
- D: working features.md is numbered bullets by Jonathan's hand, 103 lines, 1 to 104 less 20, the intro gone; row 104 says ov is retired at step 19; the plan is kb hosted by ai.md, music and ai.md renamed, its steps 1 to 19 lines gone, and kb hosted by music.md sits beside it with the same 243 lines; the journal's Current line says so. Jonathan's edit
- D: working features.md is checkbox lines, one per row of the table it was, the number kept at the start, 102 lines, 3 ticked, the intro and the plan's step 19 saying so; code_debt.py counts its 99 open, finished.py its 3 done. Jonathan's go
- D: step 18 of the plan ticked, the three hand-offs kb's, rewritten as 15 to 17 were, and the command-key new tab in Obsidian built; working features row 103; the journal has the detail; the visual report waits on Jonathan. Jonathan's go 18
...
<!-- ov's log, merged whole below on 23 September 2026: its marker was 9 September 2026, so its lines of 10 to 18 September were never settled -->
...
- D: a plugin api section in zone/music and ai.md, before tables: the two apis that meet at the dispatcher, the page's addresses and the plugin's functions, and a table of the plugin's five, specialty, listed files, is listed, labels and routes, what each takes, answers and when it is called, ov's asks going to ai's plugin until ov is retired. api in kb's lexicon
- D: step 6 rewritten in two sides, the dispatcher's and the page's, closing its four gaps: the fields answer carries each file's collection so the page can drop the six names that know the note folders, a request naming no host goes to ai's plugin, the look asks the plugin for its listing and its specialty, and saving.test's cases for the removed functions go at step 6 with them, step 19 saying so. Its proof adds a visual report
- D: the plugin api table tables the functions themselves, after egads at a routes row: four every plugin has, specialty, listed files, is listed and labels, then the specialty's own, ai's read, save, scan and strip at step 8 and music's stream at step 23, each called by a function of the dispatcher's api. Step 6, kb's lexicon and learn entry 26 say so
- D: step 6's page side keeps to the four names in Saving.ts that mirror the listing rule, and T_Bundle and project_at, the project list the projects row draws and the hierarchy hangs from, move to step 25, which replaces that row with the collection filter, since taking them at step 6 pulled step 25's work into a dispatcher step
- D: step 6's page side narrowed again, to what mirrors the listing rule, WORK_FOLDERS, reaches_under_work and the listed line inside site_of_file: file_path_of and site_of_file are the bundle design's two halves, thirty-five call sites in eight files, and go at step 25 with T_Bundle and project_at, step 25 noting that every file falls to the memory rule already. Step 19's test line follows
- D: step 6 says the plugin's import is wrapped like its calls, a plugin failing to import leaving its host's listing empty and the server up, and that the listing rule's list of note folders gains ai and kb, so their CLAUDE files are listed as step 4 promised
- D: step 6 built, the ai plugin. `ov.db.before-step-6` saved. `ai/plugin.py` with SPECIALTY, listed_files, is_listed and labels, its list of note folders gaining ai and kb. In the dispatcher PLUGINS, load_plugins, plugin_for, call_plugin, files_listed_by and listed_by in place of listed_files, is_listed_note and COLLECTIONS, the import and every call wrapped, ov's asks going to ai's plugin, the look writing the plugin's specialty on the collections rows, and the rules pass taking the plugin's labels and running with no rules too. all_fields carries each file's collection. On the page WORK_FOLDERS, reaches_under_work and the listed line in site_of_file are gone, and Fields gains collection. Proof: test_database.py 166, a stub plugin raising on one file failing that file alone, test_dispatcher.py 47, the two new CLAUDE files listed and the fields carrying collections, the listing after against before differing by ai/CLAUDE.md and kb/CLAUDE.md alone, 497 files, the always test, big picture 15, kb check clean at 541 files and 334 tests, ai 532 and building. Not looked at in a browser. The hub index, ai's map, index and log, and kb's map and log say so
- D: step 7 made ready: the three lists become the configuration's kinds, tags and tag_areas and kb's kind a word off the configured list, the fallback kind waiting for step 12, the tag_areas test going to ai at step 7 with its lists and step 19 saying so, is_design moved to step 25 with the bundle design, and a question, 7a, whether music stays among ai's six kinds
- D: ai's kinds are five, music dropped, mu's alone: 7a answered and ticked, in truth/decisions.md, step 7, the ai schema table and adopt kb
- D: mu's kinds are four, music, images, text and video, by the file's ending, in place of music for every file: in truth/decisions.md, the music specialty, its schema table, the music rules, 7a, step 22, adopt kb and mu's design and log
- D: mu's tags are four, jazz, classical, rock and hifi, by hand or by the ai, reversing no tags: in truth/decisions.md, the music specialty, its schema table, the music rules, which apply again, steps 1, 29 and 32, adopt kb and mu's design and log. Their area is a new question, 29d, the build now 29e
- D: step 7 built, the vocabulary. ai's Customizations.ts holds the five kinds, the 39 tags and the ten tag areas, and ai's main.ts hands them to kb's configuration before anything mounts. T_Kind, ALL_TAGS and TAG_AREAS left File.ts and Tag_Areas.ts, kb's kind is a word, its readers, Browse_Filters, Edit_Filters, Files.ts, Filters.ts and Labels.ts, read customizations.kinds, tags and tag_areas, area_of and tags_without_area take the lists, and KIND_UNTIL_TOLD is the word analyze until step 12. The tag areas test moved to ai, reading ai's lists through kb's functions, and kb's labels test declares the words it reads off blocks. Proof: kb check clean at 540 files and 324 tests, ai check clean at 533 files, 15 tests with the tag areas test among them, and ai builds. Not looked at in a browser. adopt kb, kb's map, index and log, and ai's map, index and log say so
- D: step 7 looked at and confirmed: the kinds row and the tag areas in ai. The plan's note and the work journal say so
- D: step 8 rewritten in the plugin api's words: ai's read, save, scan and strip are functions the dispatcher's four address functions call through call_plugin, the five pieces that move are named, labels_in_text, label_block, KNOWN_KEYS, scan_labels and strip_blocks, nothing is kept unreached since ov's asks run ai's plugin already, and the proof is one that runs
- D: step 8's two loops, scan_labels and strip_blocks, stay in the dispatcher and call the plugin's scan and strip once per file, since each reads or writes the db inside the loop, and the table says the plugin works on one file
- D: step 8 built, the dispatcher's markdown routes. read, save, scan and strip in `ai/plugin.py`, with label_block, KNOWN_KEYS, labels_in_text and without_block moved there from the dispatcher. The dispatcher's four address functions call the host's plugin through call_plugin and answer 409 where the plugin has no function or refuses; scan_labels and strip_blocks walk the listing and the db and call the plugin once per file. Proof: test_database.py 169, the block cases asking the plugin and a stub plugin lacking read and save refused, test_dispatcher.py 48 with a read through ai's plugin by name, the always test and big picture 15. Not looked at in a browser. ai's map, index and log and the hub index say so
- D: two of Jonathan's edits of lv/CLAUDE.md in the page were lost as Failed to fetch. The first met the dispatcher restarting for step 8. The second was the browser's own cap: after a write the page relates every link and logged each, one request per line, about 2,850 in a burst, and the browser refused the write among them. Found in a headless browser after python and curl showed nothing. Fixed in core's Debug.ts, lines gathered and sent in one request per file. His edits work now
- D: kb's Files.ts relates one guide's links after a write, not every guide's: the thousands of log lines per write are a handful now. In kb's map and log
- D: the dispatcher restarted through its route, the first restart since 3:56 PM: Jonathan's press of the hub page's dispatcher button had not restarted it, and the page said ✓ regardless, since it never checked. Now the dispatcher answers STARTED in /restart-status, the button reads it before and after and says ✓ only on a later time, ✗ when the old process still answers after ten seconds or nothing answers, and every action button says busy in place of doing nothing while another operation runs. The restarted dispatcher names the hosts ai and mu, live suite 48 pass. A rule added or taken away under host ai now runs on every file, as it did under ov
- D: ai.db made from ov.db through sqlite's backup, 371 files and 1116 labels, less the 23 rows of the removed memory/filter tree folder and their 5 labels: 348 files, 1111 labels, no row missing. ports.json names ai.db under ai and no db under ov, PLACE is ai.db, the default host ai, the dump ai.sql and ov.sql deleted. Both suites expect the hosts ai and mu. db suite 169 pass, always hook 4 of 4; the live suite waits for the dispatcher's restart. In truth/decisions.md
- D: the page's log after the log fix said each opened guide's links were followed again as 0, a guide with 21 links among them. kb's editor drew the next guide from nothing before its words arrived, since letting the words go kept what was drawn, and filed no links for it. Fixed in kb's Edit_Markdown.svelte, the drawing let go with the words. ov, frozen, keeps the fault
