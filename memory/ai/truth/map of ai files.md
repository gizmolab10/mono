...
- [common/Kb.ts](../../../ai/src/lib/ts/common/Kb.ts) — everything ai imports from kb, one line each, through the "kb" alias: the page, the files manager, the preferences, the operations and the app's two states. **⟵kb**
- [common/Customizations.ts](../../../ai/src/lib/ts/common/Customizations.ts) — what is true of ai and of no other host: the name. Step 5 of the plan hands it to kb as the configuration.
