...
// The registry. Holds one live backend-and-hierarchy per storage, tracks which is
...
class Databases {
	...
	// The active store's hierarchy — where the records and every operation on them live.
	...
	constructor() {
		...
		load_folds_forStorage(saved, store.hierarchy);   // its shut folders live with its own hierarchy
	...
	// One live backend-and-hierarchy per storage, built on first use.
	...
	change_storage(storage: T_Storage): void {
		...
		load_folds_forStorage(storage, store.hierarchy);   // same for the shut folders, kept by its own hierarchy
		store.hierarchy.fetch_all();
		this.active = store.db;
		preferences.write(T_Preference.db, storage);
		this.w_storage.set(storage);
		this.w_hierarchy.set(store.hierarchy);   // the active store's hierarchy changed
...
// The active store's hierarchy, two ways to reach it:
