...
export class Colors {
	...
	// Remembering these across visits is the host app's business, not core's: ov keeps them in
	// its preferences and writes them back into these stores at startup. Core holds only the
	// live values, starting at the defaults below.
	w_background_color  = stale_writable<string>('#F3F1EE');
	w_hover_color       = stale_writable<string>('rgb(220, 220, 220)');
	w_text_color	    = stale_writable<string>('black');
	w_accent_color	    = stale_writable<string>('rgb(200, 200, 200)');
	...
	/**
	 * Whenever either chosen color moves, everything derived from the pair is worked out again.
	 */
	private subscribe_to_changes() : void {
		this.w_accent_color.subscribe(() => this.settle());
		this.w_background_color.subscribe(() => this.settle());
	}
