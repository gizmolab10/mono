...
describe('reading a path in the repo back into a collection and a folder', () => {
	...
	it('reads a project\'s CLAUDE file, either spelling', () => {
		expect(site_of_file('projects/lv/CLAUDE.MD'))
			.toEqual({ bundle: T_Bundle.lv, path: 'CLAUDE.MD', is_design: false });
		expect(site_of_file('lv/CLAUDE.MD')).toBeNull();
	});

	it('reads a library\'s CLAUDE file under libraries/', () => {
		expect(site_of_file('projects/libraries/kb/CLAUDE.md'))
			.toEqual({ bundle: T_Bundle.kb, path: 'CLAUDE.md', is_design: false });
		expect(site_of_file('kb/CLAUDE.md')).toBeNull();
		expect(file_path_of(T_Bundle.core, 'CLAUDE.md')).toBe('projects/libraries/core/CLAUDE.md');
		expect(file_path_of(T_Bundle.di, 'CLAUDE.md')).toBe('projects/di/CLAUDE.md');
	});
...
describe('working out where a guide sits', () => {
	...
	it('adds the ending when the path has none, and keeps the spelling of one it has', () => {
		expect(file_path_of(T_Bundle.ji, 'roadmap')).toBe('projects/ji/roadmap.md');
		expect(file_path_of(T_Bundle.lv, 'CLAUDE.MD')).toBe('projects/lv/CLAUDE.MD');
	});
});
...
describe('working out where a folder sits', () => {
	it('answers with the collection\'s own folder when the folder has no path inside it', () => {
		...
		expect(folder_path_of(T_Bundle.di, '')).toBe('projects/di');
	});
	...
	it('adds the folder\'s path inside its collection', () => {
		...
		expect(folder_path_of(T_Bundle.di, 'zone')).toBe('projects/di/zone');
	});
});
