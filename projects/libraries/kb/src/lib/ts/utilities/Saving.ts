...
function top_of(bundle: T_Bundle): string {
	if (bundle === T_Bundle.mono) { return ''; }
	if (MEMORY_ONLY.has(bundle)) { return bundle; }
	return LIBRARIES.has(bundle) ? `projects/libraries/${bundle}` : `projects/${bundle}`;
}

// Every collection's code folder sits under projects/ since 23 September 2026, a library's one
// folder deeper under projects/libraries/. The collections below have no code folder of their own,
// so each names itself.
export const LIBRARIES = new Set<T_Bundle>([T_Bundle.core, T_Bundle.gallery, T_Bundle.kb, T_Bundle.panel]);
export const MEMORY_ONLY = new Set<T_Bundle>([T_Bundle.memory, T_Bundle.shared, T_Bundle.ov]);

// Which collection a path counting from the top of the repo begins with, and what is left of that
// path below the collection's own code folder. Nothing where the path names no collection.
function below_top(steps: string[]): { bundle: T_Bundle; rest: string[] } | null {
	for (const bundle of Object.values(T_Bundle)) {
		const top = top_of(bundle);
		if (top === '') { continue; }
		const parts = top.split('/');
		if (steps.length > parts.length && parts.every((part, at) => steps[at] === part)) {
			return { bundle, rest: steps.slice(parts.length) };
		}
	}
	return null;
}
...
export function site_of_file(where: string): File_Site | null {
	...
	if (steps[steps.length - 1].toLowerCase() === 'claude.md') {
		...
		const under = below_top(steps);
		return under && under.rest.length === 1 ? { bundle: under.bundle, path: under.rest[0], is_design: false } : null;
	}
	...
	// Below a project's own code folder, the project's; at the top of the repo, mono's.
	const under = below_top(steps);
	if (under) { return { bundle: under.bundle, path: under.rest.join('/'), is_design: false }; }
