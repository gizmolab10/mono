...
class Drag {
	 ...
	 *  Snaps to zero gap when within threshold. */
	private detect_edge_snap(): void {
		...
		for (const axis of move_axes) {
			...
			let best: { dragged_bound: Bound; value: number; distance: number } | null = null;

			for (const sibling of siblings) {
				const sibling_min = sibling.so.get_bound(min_b);
				const sibling_max = sibling.so.get_bound(max_b);

				for (const [dv, db, sv] of [
					[dragged_min, min_b, sibling_min],
					[dragged_min, min_b, sibling_max],
					[dragged_max, max_b, sibling_min],
					[dragged_max, max_b, sibling_max],
				] as [number, Bound, number][]) {
					const distance = Math.abs(dv - sv);
					if (distance < SNAP_THRESHOLD_MM && (!best || distance < best.distance)) {
						best = { dragged_bound: db, value: sv, distance };
					}
				}
			}
