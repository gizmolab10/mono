...
PROJECTS = ('', 'core', 'memory', 'panel', 'gallery', 'ai', 'di', 'ga', 'ji', 'kb', 'lv', 'ma', 'me', 'mj', 'mu', 'ov', 'ws')

# Every project's code folder sits under projects/ since 23 September 2026, a library's one
# folder deeper under projects/libraries/. So a project's CLAUDE file is projects/<name>/CLAUDE.md
# or projects/libraries/<name>/CLAUDE.md, and the repo's own is at its top.
CODE_PARENTS = ('projects', 'projects/libraries')


def code_folder(root, project):
    """Where a project's code sits: under projects/, a library's under projects/libraries/, and the
    repo's own top for the empty name. A project with no code folder of its own answers a path
    that is not there, which the caller passes over."""
    if not project:
        return root
    for parent in CODE_PARENTS:
        whole = os.path.join(root, *parent.split('/'), project)
...
def listed_files(root):
    ...
    for project in PROJECTS:
        ...
        top_dir = code_folder(root, project)
...
def is_listed(root, where):
    ...
    depth. Every project's CLAUDE file sits under projects/, a library's under
    projects/libraries/."""
    ...
    if parts[-1].lower() == 'claude.md':
        if len(parts) > 1:
            return '/'.join(parts[:-2]) in CODE_PARENTS and parts[-2] in PROJECTS
        return True
