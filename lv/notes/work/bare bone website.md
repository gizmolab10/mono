...
2. **A parser.** Turns one md file's text into something the browser can render. It has to handle:
    ...
    - The stack: `unified` plus `remark-parse` (standard markdown), `remark-frontmatter` (the top three-dashed block), `@portaljs/remark-wiki-link` (`[[Other Note]]` and the embed form `![[name.png]]`), `remark-callout` (`> [!note]` blocks), then `remark-rehype` plus `rehype-stringify` to produce the HTML the page can drop in. Every piece pulls from the same package registry yarn uses. The wiki-link library accepts our name-resolver, so links route through the loader's map.
