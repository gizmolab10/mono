...
run_step "Path Validator" "npx tsx tools/validate-paths.ts" || FAILED_STEPS+=("Path Validator")
run_step "Workspace Integrity" "yarn workspaces info" || FAILED_STEPS+=("Workspace Integrity")
run_step "Build (ws)" "cd ~/GitHub/mono/projects/ws && yarn build" || FAILED_STEPS+=("Build (ws)")
run_step "Build (di)" "cd ~/GitHub/mono/projects/di && yarn build" || FAILED_STEPS+=("Build (di)")

if [[ $FULL -eq 1 ]]; then
  run_step "Docs Build" "cd ~/GitHub/mono && yarn docs:build" || FAILED_STEPS+=("Docs Build")
  run_step "Tests (ws)" "cd ~/GitHub/mono/projects/ws && yarn test --run" || FAILED_STEPS+=("Tests (ws)")
  run_step "Tests (di)" "cd ~/GitHub/mono/projects/di && yarn test --run" || FAILED_STEPS+=("Tests (di)")
fi
