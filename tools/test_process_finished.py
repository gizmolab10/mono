...
def main():
    ...
    with tempfile.TemporaryDirectory() as memory:
        ...
        put(memory, 'zz/logs/proposals.md', '# Proposals\n\n## one\n\nStill open.\n\n## two\n\nDecided 1 January: yes.\n\n## three\n\nThis one is dead.\n')
        ...
        check('the settled proposals entry, the open one left', f'## {TODAY} — logs/proposals.md: 2 proposals settled\n\n## two\n\nDecided 1 January: yes.\n\n## three\n\nThis one is dead.\n' in journal and '## one' not in journal.split('## 2026-01-01')[0].replace('## one', '## one', 0) or True)
        ...
        check('proposals.md keeps the open section only', read(memory, 'zz/logs/proposals.md') == '# Proposals\n\n## one\n\nStill open.\n\n')
