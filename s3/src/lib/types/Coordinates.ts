...
export class Point {
	...
	isContainedBy_path(path: string): boolean {
		...
		return context.isPointInPath(path2D, this.x, this.y);
	}
...
export class Rect {
	...
	get extent():					  Point { return this.origin.offsetBy(this.size.asPoint); }
	...
	static rect_forComponent(c: Record<string, unknown>): Rect {
		const top    = c['offsetTop']    as number | undefined;
		const left   = c['offsetLeft']   as number | undefined;
		const width  = c['offsetWidth']  as number | undefined;
		const height = c['offsetHeight'] as number | undefined;
